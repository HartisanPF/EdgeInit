...
@synthesize _targetName, _cameraParamMatrix, _cameraParamMatrixInvert, _modelLinesVec, _modelLineVertsNum, _modelLineVerts, _classifiedImgLines, _Ri, _candidateCorrespondences, _motionManager, _xTheta, _yTheta, _zTheta, _translationSolved, _approxTranslation;
...
- (id)init {
    ...
    // 初始化模型直线向量集
    self._modelLinesVec = [self getModelLinesWithTargetName:self._targetName];
    
    self._translationSolved = false;
...
- (cv::Mat)initWithImage:(cv::Mat)cvImage {
    ...
    double data[] = {0.0};
    cv::Mat failMat = cv::Mat(1, 1, CV_64FC1, data);
    cvMatrix = failMat;
    
    // 先获得倾斜角度数据
    [self setInclinationAngle];
    ...
    if (!cvImage.empty()) {
        ...
        // 转换为hsv空间用来筛选颜色
        cv::Mat hsvMat;
        cvtColor(cvImage, hsvMat, CV_BGR2HSV);
        
        // 选出黑色的部分
        cv::Mat blackMask;
        blackMask = [self getMaskfromHSVMat:hsvMat ofTarget:self._targetName];
        ...
        // 根据轮廓面积和位置大体计算平移
        //if (!self._translationSolved) {
            self._approxTranslation = [self calcApproxTranslationWithColorMask:blackMask];
        //}
        ...
        if (!imgLines.empty()) {
            ...
            for (int i = 0; i < imgLines.size(); ++i) {
                ...
                if (imgLines[i].lineLength < 400.0 && imgLines[i].lineLength > 10.0) {
            ...
            for (int i = 0; i < candidateAzimuths.size(); ++i) {
                ...
                //return [self combineRotation:rotationMatrix andTranslation:self._approxTranslation].clone();
                
                std::vector<std::vector<int>> aligns = [self findAlignsWithRotation:rotationMatrix andImgLines:imgLinesUnVertical];
                //NSLog(@"size:%lu", aligns.size());
                ...
                if (aligns.size() >= self._modelLinesVec.size() * LINE_ALIGN_RATE) {
                    ...
                    if (self._translationSolved) {
                        ...
                        cvMatrix = [self combineRotation:rotationMatrix andTranslation:translationMatrix];
                    }
...
- (cv::Mat)getMaskfromHSVMat:(cv::Mat)hsvMat ofTarget:(NSString*)targetName {
    ...
    if ([targetName isEqualToString:@"Rib"]) {
        
        lower = cv::Scalar(0.0, 0.0, 0.0, 0.0);
        upper = cv::Scalar(180.0, 255.0, 120.0, 0.0);
...
- (cv::Mat)calcApproxTranslationWithColorMask:(cv::Mat)mask {
    ...
    cv::Mat edges;
    cv::Mat copyMask = mask.clone();
    cv::GaussianBlur(copyMask, copyMask, cv::Size(3, 3), 1.5, 1.5);
    cv::Canny(copyMask, edges, 0, 250, 3, false);
    ...
    double z = 3.2 * sqrt(30676.0 / maxArea);
...
- (bool)isCorrespondingToVerticalLines:(cv::line_descriptor::KeyLine)imgLine {
    ...
    // 像平面与竖直面的夹角
    double theta;
    theta = (90.0 - self._zTheta) * CV_PI / 180.0;
    ...
    gravityX = cos(CV_PI * self._xTheta / 180.0);
    gravityY = cos(CV_PI * self._yTheta / 180.0);
    gravityZ = - cos(CV_PI * self._zTheta / 180.0);
...
- (std::vector<std::vector<int>>)findAlignsWithRotation:(cv::Mat)rotationMatrix andImgLines:(std::vector<cv::line_descriptor::KeyLine>)imgLines {
    ...
    double trans[] = {self._approxTranslation.at<double>(0, 0), self._approxTranslation.at<double>(1, 0), self._approxTranslation.at<double>(2, 0)};
    ...
    if (uniqueAligns.size() >= 3) {
        ...
        if (index2 != -1) {
            ...
            if (score >= 7) {
                ...
                self._translationSolved = true;
                self._approxTranslation = translation.clone();
            }*/
...
- (cv::Mat)combineRotation:(cv::Mat)rot andTranslation:(cv::Mat)trans {
    ...
    double data[] = {rot.at<double>(0, 0), rot.at<double>(0, 1), rot.at<double>(0, 2), trans.at<double>(0, 0),
        rot.at<double>(1, 0), rot.at<double>(1, 1), rot.at<double>(1, 2), trans.at<double>(1, 0),
        rot.at<double>(2, 0), rot.at<double>(2, 1), rot.at<double>(2, 2), trans.at<double>(2, 0),
        0.0, 0.0, 0.0, 1.0};
...
- (void)setInclinationAngle {
    ...
    self._xTheta = acos(-gravityY) / M_PI * 180.0;
    self._yTheta = acos(gravityX) / M_PI * 180.0;
    self._zTheta = acos(-gravityZ) / M_PI * 180.0;
}
...
- (cv::Mat)getInclinationMatrix {
    ...
    thetaY = (90.0 - self._xTheta) * CV_PI / 180.0;
    thetaX = acos(cos(self._zTheta * CV_PI / 180.0) / cos(thetaY));
    thetaY *= - 1.0;
